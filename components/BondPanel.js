...
function handlePrintAll({ sections, lastUpdate }) {
  ...
  const tableRows = (data) => data.map(b => `<tr><td>${b.emisor ?? "—"}</td><td>${b.cupon !== null ? b.cupon.toFixed(3)+"%" : "—"}</td><td>${b.vencimiento}</td><td style="text-align:right;color:#888">${b.bid?.toFixed(2) ?? "—"}</td><td style="text-align:right">${b.precio?.toFixed(2) ?? "—"}</td><td style="text-align:right;font-weight:600">${b.tir != null ? b.tir.toFixed(2)+"%" : "—"}</td></tr>`).join("");
  let body = "";
  sectionKeys.forEach((key, idx) => {
    ...
    body += `<table><thead><tr><th>Emisor</th><th>Cupón</th><th>Vencimiento</th><th style="text-align:right">Bid</th><th style="text-align:right">Ask</th><th style="text-align:right">TIR</th></tr></thead><tbody>${tableRows(data)}</tbody></table>`;
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.65)" }} onClick={onClose}>
      <div className="rounded-xl p-6 w-96 text-white" style={{ backgroundColor: "#0F1520", border: "1px solid #2A3A50" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-4 mb-5 text-sm">
          {[["Cupón anual", bond.cupon?.toFixed(3)+"%"], ["Vencimiento", bond.vencimiento], ["Ask", (bond.ask ?? bond.precio)?.toFixed(2) ?? "—"]].map(([lbl, val]) => (
            ...
            <table style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#0d0d1a" }}>
                  ...
                  {["Bid","Ask","TIR"].map(h => (<th key={h} className="py-2 px-4 text-right text-xs font-bold text-[#94A3B8] uppercase tracking-wide whitespace-nowrap">{h}</th>))}
