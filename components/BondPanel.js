...
function handlePrintAll({ sections, lastUpdate }) {
  ...
  sectionKeys.forEach((key) => {
    ...
    body += `<table>${tableHead}<tbody>${tableRows(data)}</tbody></table></div>`;
  });
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Reporte Completo de Bonos</title><style>${PRINT_CSS}</style></head><body><div class="header"><div><h1>Market Tracker — Reporte Completo de Bonos</h1><p>Fecha: ${fecha}${lastUpdate ? " · Última actualización: "+lastUpdate : ""}</p></div><div class="logo">Gastón Bengochea<span>Corredor de Bolsa</span></div></div>${body}<div class="footer"><p>Gastón Bengochea Corredor de Bolsa · Precios indicativos, no constituyen oferta de compraventa</p><p>${fecha}</p></div><script>window.onload=()=>window.print();<\/script></body></html>`;
  const win = window.open("", "_blank");
  if (win) { win.document.write(html); win.document.close(); }
}
