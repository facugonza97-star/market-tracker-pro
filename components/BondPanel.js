...
function handlePrintAll({ sections, lastUpdate }) {
  ...
  const tableRows = (data) => data.map(b => `<tr><td>${b.emisor ?? "—"}</td><td>${b.cupon !== null ? b.cupon.toFixed(3)+"%" : "—"}</td><td>${b.vencimiento}</td><td>${b.bid?.toFixed(2) ?? "—"}</td><td>${b.ask?.toFixed(2) ?? "—"}</td><td>${b.tir != null ? b.tir.toFixed(2)+"%" : "—"}</td></tr>`).join("");
  const tableHead = `<thead><tr><th>Emisor</th><th>Cupón</th><th>Vencimiento</th><th>Precio Compra</th><th>Precio Venta</th><th>TIR</th></tr></thead>`;
  const printAllCSS = `*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:9px;color:#1a1a2e;padding:16px 24px}.header{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;padding-bottom:10px;border-bottom:2px solid #1a1a2e}.header-left h1{font-size:18px;font-weight:700;margin:0 0 2px;color:#1a1a2e}.header-left p{font-size:8px;color:#666;margin:0}.logo{text-align:right}.logo-name{font-size:13px;font-weight:700;color:#1a1a2e}.logo-sub{font-size:8px;color:#888;letter-spacing:.5px;display:block}.section-block{margin-bottom:16px}.section-header{background-color:#1a1a2e;color:white;padding:5px 10px;font-size:10px;font-weight:700;letter-spacing:.3px}.chart-area{background:#f5f7fa;border-left:.5px solid #ddd;border-right:.5px solid #ddd;border-bottom:.5px solid #ddd;height:110px;margin-bottom:0}table{width:100%;border-collapse:collapse;font-size:8.5px}thead tr{background-color:#e8ecf0}thead th{padding:4px 6px;text-align:left;font-size:8px;font-weight:700;color:#444;text-transform:uppercase;letter-spacing:.3px;border-bottom:1px solid #1a1a2e}thead th:last-child{text-align:right}thead th:nth-last-child(2),thead th:nth-last-child(3){text-align:right}tbody tr:nth-child(even){background-color:#f5f7fa}tbody tr{border-bottom:.5px solid #e8e8e8}tbody td{padding:2.5px 6px;color:#1a1a2e}tbody td:last-child{text-align:right;font-weight:700;color:var(--tir-color,#185FA5)}tbody td:nth-last-child(2),tbody td:nth-last-child(3){text-align:right;font-family:monospace}.footer{margin-top:12px;padding-top:6px;border-top:1px solid #ccc;display:flex;justify-content:space-between}.footer p{font-size:7.5px;color:#999;margin:0}@media print{body{padding:10px 16px}@page{margin:.5cm;size:A4}}`;
  let body = "";
  sectionKeys.forEach((key) => {
    const cfg = SECTION_CONFIG[key] || { label: key, color: "#5B8DEF" };
    const label = stripEmoji(cfg.label);
    const data = sections[key];
    body += `<div class="section-block"><div class="section-header">${label}</div>`;
    ...
    body += `<table>${tableHead}<tbody style="--tir-color:${cfg.color}">${tableRows(data)}</tbody></table></div>`;
  });
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Reporte Completo de Bonos</title><style>${printAllCSS}</style></head><body><div class="header"><div class="header-left"><h1>Reporte Completo de Bonos</h1><p>Gastón Bengochea Corredor de Bolsa · Fecha: ${fecha}${lastUpdate ? " · Actualización: "+lastUpdate : ""} · Precios indicativos</p></div><div class="logo"><div class="logo-name">Gastón Bengochea</div><span class="logo-sub">Corredor de Bolsa</span></div></div>${body}<div class="footer"><p>Gastón Bengochea Corredor de Bolsa · Precios indicativos, no constituyen oferta de compraventa</p><p>${fecha}</p></div><script>window.onload=()=>window.print();<\/script></body></html>`;
  const win = window.open("", "_blank");
  if (win) { win.document.write(html); win.document.close(); }
}
