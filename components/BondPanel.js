...
function handlePrint({ activeData, activeConfig, compareData, compareConfig, isComparing, lastUpdate }) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${activeLabel} — Curva de Rendimiento</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:10px;color:#1a1a2e;padding:20px 28px}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;padding-bottom:10px;border-bottom:1.5px solid #1a1a2e}.header h1{font-size:14px;font-weight:700;margin-bottom:3px}.header p{font-size:10px;color:#666}.logo{font-size:14px;font-weight:700;text-align:right}.logo span{display:block;font-size:9px;font-weight:400;color:#888}.section-title{font-size:8px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px}.chart-area{background:#f8f9fb;border:.5px solid #e8e8e8;border-radius:6px;height:140px;margin-bottom:10px;overflow:hidden}table{width:100%;border-collapse:collapse;font-size:9px;margin-bottom:4px}thead tr{border-bottom:1px solid #1a1a2e}thead th{padding:3px 5px;text-align:left;font-size:8px;font-weight:700;color:#555;text-transform:uppercase}tbody tr{border-bottom:.5px solid #f0f0f0}tbody td{padding:2px 5px}.section-sep{margin-top:10px}.footer{margin-top:10px;padding-top:6px;border-top:.5px solid #e0e0e0;display:flex;justify-content:space-between}.footer p{font-size:9px;color:#aaa}@media print{body{padding:16px 20px}@page{margin:.6cm;size:A4}}</style></head><body><div class="header"><div><h1>${activeLabel}${isComparing ? " — comparado con "+compareLabel : ""}</h1><p>Curva de Rendimiento · Fecha: ${fecha}${lastUpdate ? " · Actualización: "+lastUpdate : ""} · Precios indicativos</p></div><div class="logo">Gastón Bengochea<span>Corredor de Bolsa</span></div></div><div class="section-title">Curva de rendimiento</div><div class="chart-area"><svg viewBox="0 0 700 160" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">${buildCurveSVG(activeData, activeConfig.color, activeLabel, compareData, compareConfig?.color, compareLabel, isComparing)}</svg></div><div class="section-title">${activeLabel} — Listado de bonos</div><table><thead><tr><th>Emisor</th><th>Cupón</th><th>Vencimiento</th><th style="text-align:right">Precio Compra</th><th style="text-align:right">Precio Venta</th><th style="text-align:right">TIR</th></tr></thead><tbody>${tableRows(activeData, activeConfig.color)}</tbody></table>${isComparing && compareData ? `<div class="section-sep"><div class="section-title">${compareLabel} — Listado de bonos</div><table><thead><tr><th>Emisor</th><th>Cupón</th><th>Vencimiento</th><th style="text-align:right">Precio Compra</th><th style="text-align:right">Precio Venta</th><th style="text-align:right">TIR</th></tr></thead><tbody>${tableRows(compareData, compareConfig.color)}</tbody></table></div>` : ""}<div class="footer"><p>Gastón Bengochea Corredor de Bolsa · Precios indicativos, no constituyen oferta de compraventa</p><p>${fecha}</p></div><script>window.onload=()=>window.print();<\/script></body></html>`;
...
function handlePrintAll({ sections, lastUpdate }) {
  ...
  const tableRows = (data) => data.map(b => `<tr><td>${b.emisor ?? "—"}</td><td>${b.cupon !== null ? b.cupon.toFixed(3)+"%" : "—"}</td><td>${b.vencimiento}</td><td style="text-align:right;color:#888">${b.bid?.toFixed(2) ?? "—"}</td><td style="text-align:right">${b.ask?.toFixed(2) ?? "—"}</td><td style="text-align:right;font-weight:600">${b.tir != null ? b.tir.toFixed(2)+"%" : "—"}</td></tr>`).join("");
  let body = "";
  sectionKeys.forEach((key, idx) => {
    ...
    if (key !== "T-bills") body += `<div class="chart-area"><svg viewBox="0 0 700 160" width="100%" height="100%" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">${buildCurveSVG(data, cfg.color, label, null, null, "", false)}</svg></div>`;
    body += `<table><thead><tr><th>Emisor</th><th>Cupón</th><th>Vencimiento</th><th style="text-align:right">Precio Compra</th><th style="text-align:right">Precio Venta</th><th style="text-align:right">TIR</th></tr></thead><tbody>${tableRows(data)}</tbody></table>`;
  });
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>Reporte Completo de Bonos</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;font-size:10px;color:#1a1a2e;padding:20px 28px}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px;padding-bottom:10px;border-bottom:1.5px solid #1a1a2e}.header h1{font-size:14px;font-weight:700;margin-bottom:3px}.header p{font-size:10px;color:#666}.logo{font-size:14px;font-weight:700;text-align:right}.logo span{display:block;font-size:9px;font-weight:400;color:#888}.emisor-title{font-size:12px;font-weight:700;margin-bottom:4px}.chart-area{background:#f8f9fb;border:.5px solid #e8e8e8;border-radius:6px;height:140px;margin-bottom:10px;overflow:hidden}table{width:100%;border-collapse:collapse;font-size:9px;margin-bottom:4px}thead tr{border-bottom:1px solid #1a1a2e}thead th{padding:3px 5px;text-align:left;font-size:8px;font-weight:700;color:#555;text-transform:uppercase}tbody tr{border-bottom:.5px solid #f0f0f0}tbody td{padding:2px 5px}.sep{border-top:.5px solid #ddd;margin:10px 0}.footer{margin-top:10px;padding-top:6px;border-top:.5px solid #e0e0e0;display:flex;justify-content:space-between}.footer p{font-size:9px;color:#aaa}@media print{body{padding:16px 20px}@page{margin:.6cm;size:A4}}</style></head><body><div class="header"><div><h1>Market Tracker — Reporte Completo de Bonos</h1><p>Fecha: ${fecha}${lastUpdate ? " · Última actualización: "+lastUpdate : ""}</p></div><div class="logo">Gastón Bengochea<span>Corredor de Bolsa</span></div></div>${body}<div class="footer"><p>Gastón Bengochea Corredor de Bolsa · Precios indicativos, no constituyen oferta de compraventa</p><p>${fecha}</p></div><script>window.onload=()=>window.print();<\/script></body></html>`;
  const win = window.open("", "_blank");
  if (win) { win.document.write(html); win.document.close(); }
}
