...
function handlePrint({ activeData, activeConfig, compareData, compareConfig, isComparing, lastUpdate }) {
  ...
  const buildCurveSVG = (data1, color1, data2, color2, comparing) => {
    ...
    const px = y => maxX === minX ? 350 : 40 + ((y - minX) / (maxX - minX)) * 620;
    ...
    let svg = `<polyline points="${pts1}" fill="none" stroke="${color1}" stroke-width="2.5"/>`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.65)" }} onClick={onClose}>
      <div className="rounded-xl p-6 w-96 text-white" style={{ backgroundColor: "#0F1520", border: "1px solid #2A3A50" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-4 mb-5 text-sm">
          {[["Cupón anual", bond.cupon?.toFixed(3)+"%"], ["Vencimiento", bond.vencimiento], ["Precio mercado", bond.precio?.toFixed(2) ?? "—"]].map(([lbl, val]) => (
            ...
            <table style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ backgroundColor: "#0d0d1a" }}>
                  ...
                  {["Precio","TIR"].map(h => (<th key={h} className="py-2 px-4 text-right text-xs font-bold text-[#94A3B8] uppercase tracking-wide whitespace-nowrap">{h}</th>))}
