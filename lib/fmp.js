...
// Devuelve el array de price-change para un conjunto de símbolos, o null si la
// request falla / FMP no devuelve un array (rate-limit, símbolo que rompe el batch, etc.)
async function fetchPriceChangeChunk(symbols) {
  if (!symbols.length) return [];
  try {
    const res = await fetch(
      `${STABLE}/stock-price-change?symbol=${symbols.join(",")}&apikey=${API_KEY}`,
      { next: { revalidate: 300 } }
    );
    if (!res.ok) return null;
    const data = await res.json();
...
export async function fetchStockPriceChange(tickers) {
  ...
  for (let i = 0; i < normalTickers.length; i += 50) {
    const chunk = normalTickers.slice(i, i + 50);
    let data = await fetchPriceChangeChunk(chunk);
    if (data === null) {
      // El batch falló: degradar a request por símbolo para que un único
      // problema (rate-limit o símbolo conflictivo) no borre los períodos de todos.
      const perSymbol = await Promise.all(chunk.map((s) => fetchPriceChangeChunk([s])));
      data = perSymbol.filter(Boolean).flat();
    }
    normalResults.push(...data);
  }
